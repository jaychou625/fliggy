package com.webbeds.fliggy.entity;

import lombok.*;

import java.util.Date;

@NoArgsConstructor
@AllArgsConstructor
@ToString
public class Fliggy_roomType_info {
    @Getter @Setter private String outer_id;
    @Getter @Setter private String name;
    @Getter @Setter private String max_occupancy;
    @Getter @Setter private String area;
    @Getter @Setter private String floor;
    @Getter @Setter private String bed_type;
    @Getter @Setter private String bed_size;
    @Getter @Setter private String internet;
    @Getter @Setter private String service;
    @Getter @Setter private String extend;
    @Getter @Setter private String window_type;
    @Getter @Setter private String srid;
    @Getter @Setter private String out_hid;
    @Getter @Setter private String vendor = "DOTW";
    @Getter @Setter private String pics;
    @Getter @Setter private String name_e;
    @Getter @Setter private String operator;
    @Getter @Setter private String connection_type;
    @Getter @Setter private String name_before;
    @Getter @Setter private String name_after;
    @Getter @Setter private String name_final;
    @Getter @Setter private String state;
    @Getter @Setter private Date insertDate;
    @Getter @Setter private String error_msg;
    @Getter @Setter private String mapping;
}
